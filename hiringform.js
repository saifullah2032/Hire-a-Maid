  ...
  // Shift time validation elements
  ...
  // Fetch service categories with cost
  async function fetchCategories() {
      try {
          ...
          querySnapshot.forEach((docSnap) => {
              ...
              option.textContent = `${data.name} - ₹${data.cost}`;
  ...
  // Work Shift Time Logic
  shiftTo.disabled = true;
  shiftFrom.addEventListener("change", function () {
      if (this.value) {
          let [hours, minutes] = this.value.split(":").map(Number);
          hours += 1; // Add 1 hour
          if (hours >= 24) hours -= 24; // Handle midnight rollover
          const minHour = String(hours).padStart(2, "0");
          const minMinutes = String(minutes).padStart(2, "0");
          shiftTo.min = `${minHour}:${minMinutes}`;
          shiftTo.disabled = false;
      } else {
          shiftTo.disabled = true;
          shiftTo.value = "";
      }
  ...
  hiringForm.addEventListener("submit", async (event) => {
      ...
      if (shiftFrom.value && shiftTo.value) {
          const fromTime = new Date(`1970-01-01T${shiftFrom.value}:00`);
          const toTime = new Date(`1970-01-01T${shiftTo.value}:00`);
          if (toTime <= fromTime) {
              toTime.setDate(toTime.getDate() + 1);
          }
          const diffHours = (toTime - fromTime) / (1000 * 60 * 60);
          if (diffHours < 1) {
              alert("Work Shift To must be at least 1 hour after Work Shift From.");
              return;
          }
      }
      ...
      const serviceCost = selectedOption ? selectedOption.dataset.cost : 0;
      ...
      window.tempBookingData = {
          ...
          cost: serviceCost,
          shift_from: shiftFrom.value || "Not Provided",
          shift_to: shiftTo.value || "Not Provided",
      ...
      qrCodeDiv.innerHTML = "";
      const upiLink = `upi://pay?pa=test@upi&pn=HouseMaid&am=${serviceCost}&cu=INR&tn=Booking-${Date.now()}`;
      new QRCode(qrCodeDiv, { text: upiLink, width: 200, height: 200 });
      alert(`Scan the QR code and enter your Transaction ID. Amount: ₹${serviceCost}`);
  });
